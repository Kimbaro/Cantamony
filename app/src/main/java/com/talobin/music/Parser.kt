...
//            Log.d(TAG, "Successfully parsed a file $data")
    ...
    fun parseString(xmlString: String): ScorePartWise? {
        try {
            ...
            if (data != null) {
                Log.d(TAG, "Successfully parsed a string. Parts count: ${data.parts?.size ?: 0}")
            } else {
                Log.w(TAG, "Parsed string but result is null")
            }
            return data
        } catch (e: Exception) {
            Log.e(TAG, "Failed to parse String. Exception: ${e.message}", e)
            Log.e(TAG, "XML string length: ${xmlString.length}")
            Log.e(TAG, "XML string first 500 chars: ${xmlString.take(500)}")
            return null
        }
    }
    //endregion
        ...
        if (scorePartWise.parts != null && scorePartWise.parts!!.size > partIndex) {
            ...
            if (measureList != null && measureList.size > 0) {
                for (i in measureList.indices) {
                    val eachMeasure = measureList[i]
                    if (eachMeasure != null) {
                        val noteList = eachMeasure.noteList
